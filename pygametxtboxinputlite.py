...
class TextInputBoxLite:
    ...
    This class is lite version of original class.
    It blits your text directly on the main surface passed by user for performance.
    Because creating surface with large width and height results into performance drop.
    """
    def __init__(
            self,
            screen,
            ...
            password=False):
        """
        :param screen: Main pygame surface on which you want to blit the text
    ...
    def update(self, events, cursor_visible=True):
        ...
        if self.cursor_visible:
            self.screen.blit(self.cursor_surface, (cursor_x_pos, cursor_y_pos))
    ...
    def render_surface(self):
        if not len(self.input_string):
            return (self.x, self.y) if self.align_text == "left" else (self.x + self.max_width/2, self.y)
        ...
        if self.max_width == -1:
            ...
            font_surface = self.font_object.render(string, self.antialias, self.text_color)
            self.screen.blit(font_surface, (self.x, self.y))
            cursor_x_pos = self.font_object.size(string[:self.cursor_position])[0] + self.x

            return (cursor_x_pos, self.y)
